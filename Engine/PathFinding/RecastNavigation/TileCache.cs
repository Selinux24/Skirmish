...
namespace Engine.PathFinding.RecastNavigation
{
    ...
    public class TileCache
    {
        ...
        public CompressedTile AddTile(TileCacheData data, TileFlags flags)
        {
            ...
            int h = NavMeshUtils.ComputeTileHash(header.tx, header.ty, m_tileLutMask);
        ...
        private CompressedTile GetTileAt(int tx, int ty, int tlayer)
        {
            ...
            int h = NavMeshUtils.ComputeTileHash(tx, ty, m_tileLutMask);
        ...
        private bool BuildNavMeshTile(CompressedTile tile, NavMesh navmesh)
        {
            ...
            var param = new NavMeshCreateParams
            {
                ...
                nvp = Constants.DT_VERTS_PER_POLYGON,
        ...
        private static bool BuildTileCachePolyMesh(TileCacheContourSet lcset, out TileCachePolyMesh mesh)
        {
            ...
            for (int i = 0; i < lcset.nconts; ++i)
            {
                ...
                // Merge polygons.
                int maxVertsPerPoly = Constants.DT_VERTS_PER_POLYGON;
                ...
                {
                    var p = new Polygoni(Constants.DT_VERTS_PER_POLYGON * 2);//Polygon with adjacency
                    var q = polys[j];
                    for (int k = 0; k < Constants.DT_VERTS_PER_POLYGON; ++k)
                    {
                        p[k] = q[k];
                    }
