...
namespace Engine.PathFinding.RecastNavigation.Recast
{
    ...
    static class RecastUtils
    {
        ...
        public static bool ErodeWalkableArea(int radius, CompactHeightfield chf)
        {
            ...
            for (int y = 0; y < h; ++y)
            {
                for (int x = 0; x < w; ++x)
                {
                    ...
                    for (int i = c.Index, ni = (c.Index + c.Count); i < ni; ++i)
                    {
                        if (chf.Areas[i] == AreaTypes.RC_NULL_AREA)
                        {
                            dist[i] = 0;
                        }
                        else
                        {
                            ...
                            for (int dir = 0; dir < 4; ++dir)
                            {
                                if (GetCon(s, dir) != RC_NOT_CONNECTED)
                                {
                                    ...
                                    if (chf.Areas[nidx] != AreaTypes.RC_NULL_AREA)
                                    {
                                        nc++;
                                    }
                                }
            ...
            for (int i = 0; i < chf.SpanCount; ++i)
            {
                if (dist[i] < thr)
                {
                    chf.Areas[i] = AreaTypes.RC_NULL_AREA;
                }
            }
        ...
        public static void InsertSort(AreaTypes[] a, int n)
        {
            int i, j;
            for (i = 1; i < n; i++)
            {
        ...
        public static bool MedianFilterWalkableArea(CompactHeightfield chf)
        {
            ...
            // Init distance.
            AreaTypes[] areas = Helper.CreateArray(chf.SpanCount, (AreaTypes)0xff);
                    ...
                    for (int i = c.Index, ni = (c.Index + c.Count); i < ni; ++i)
                    {
                        ...
                        for (int dir = 0; dir < 4; ++dir)
                        {
                            if (GetCon(s, dir) != RC_NOT_CONNECTED)
                            {
                                ...
                                if (chf.Areas[ai] != AreaTypes.RC_NULL_AREA)
                                {
                                ...
                                if (GetCon(a, dir2) != RC_NOT_CONNECTED)
                                {
                                    ...
                                    if (chf.Areas[ai2] != AreaTypes.RC_NULL_AREA)
                                    {
        ...
        /// </remarks>
        public static void MarkBoxArea(Vector3 bmin, Vector3 bmax, AreaTypes areaId, CompactHeightfield chf)
        {
            ...
            for (int z = minz; z <= maxz; ++z)
            {
                for (int x = minx; x <= maxx; ++x)
                {
                    ...
                    {
                        var s = chf.Spans[i];
                        if (s.Y >= miny && s.Y <= maxy && chf.Areas[i] != AreaTypes.RC_NULL_AREA)
                        {
                            chf.Areas[i] = areaId;
                        }
                    }
        ...
        /// </remarks>
        public static void MarkConvexPolyArea(Vector3[] verts, int nverts, float hmin, float hmax, AreaTypes areaId, CompactHeightfield chf)
        {
        ...
        /// The value of spacial parameters are in world units.
        /// </remarks>
        public static void MarkCylinderArea(Vector3 pos, float r, float h, AreaTypes areaId, CompactHeightfield chf)
        {
            ...
            if (fillStack)
            {
                ...
                for (int y = 0; y < h; ++y)
                {
                    for (int x = 0; x < w; ++x)
                    {
                        ...
                        for (int i = c.Index, ni = (c.Index + c.Count); i < ni; ++i)
                        {
                            if (chf.Dist[i] >= level && srcReg[i] == 0 && chf.Areas[i] != AreaTypes.RC_NULL_AREA)
                            {
                                stack.Add(new LevelStackEntry { X = x, Y = y, Index = i });
                            }
                        }
        ...
        private static void SortCellsByLevel(int startLevel, CompactHeightfield chf, int[] srcReg, int nbStacks, List<List<LevelStackEntry>> stacks, int loglevelsPerStack) // the levels per stack (2 in our case) as a bit shift
        {
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    for (int i = c.Index, ni = (c.Index + c.Count); i < ni; i++)
                    {
                        if (chf.Areas[i] == AreaTypes.RC_NULL_AREA || srcReg[i] != 0)
                        {
                            continue;
                        }
        ...
        public static bool AddSpan(Heightfield hf, int x, int y, int smin, int smax, AreaTypes area, int flagMergeThr)
        {
        ...
        private static void MergeRegionHoles(ContourRegion region)
        {
            ...
            Array.Sort(region.Holes, ContourHole.DefaultComparer);
                ...
                for (int x = borderSize; x < w - borderSize; ++x)
                {
                    ...
                    for (int i = c.Index, ni = (c.Index + c.Count); i < ni; ++i)
                    {
                        ...
                        if (GetCon(s, 0) != RC_NOT_CONNECTED)
                        {
                            ...
                            int ay = y + GetDirOffsetY(0);
                            int ai = chf.Cells[ax + ay * w].Index + GetCon(s, 0);
                            if (chf.Areas[ai] != AreaTypes.RC_NULL_AREA && srcReg[ai] != 0xff)
                            {
                                sid = srcReg[ai];
                            }
                        }
                        ...
                        for (int j = c.Index, nj = (c.Index + c.Count); j < nj; ++j)
                        {
                            ...
                            for (int dir = 0; dir < 4; ++dir)
                            {
                                if (GetCon(s, dir) != RC_NOT_CONNECTED)
                                {
                                    ...
                                    // Portal mask
                                    if (chf.Areas[ai] != AreaTypes.RC_NULL_AREA && lid != alid)
                                    {
                                        portal |= (1 << dir);
        ...
        public static int CountPolyVerts(IndexedPolygon p)
        {
        ...
        public static int GetPolyMergeValue(IndexedPolygon pa, IndexedPolygon pb, Int3[] verts, out int ea, out int eb)
        {
            ea = -1;
            eb = -1;
        ...
        public static IndexedPolygon MergePolys(IndexedPolygon pa, IndexedPolygon pb, int ea, int eb)
        {
            int na = CountPolyVerts(pa);
            int nb = CountPolyVerts(pb);
        ...
        private static bool RemoveVertex(PolyMesh mesh, int rem, int maxTris)
        {
            ...
            int ntris = Triangulate(tverts, ref thole, out Int3[] tris);
            if (ntris < 0)
            {
        ...
        public static bool BuildPolyMesh(ContourSet cset, int nvp, out PolyMesh mesh)
        {
            ...
            for (int i = 0; i < cset.nconts; ++i)
            {
                ...
                int ntris = Triangulate(cont.verts, ref indices, out Int3[] tris);
                if (ntris <= 0)
                {
        ...
        public static bool CopyPolyMesh(PolyMesh src, out PolyMesh dst)
        {
            dst = new PolyMesh
            {
                ...
                Polys = new IndexedPolygon[src.NPolys],
        ...
        private static void SeedArrayWithPolyCenter(CompactHeightfield chf, IndexedPolygon poly, int npoly, Int3[] verts, int bs, HeightPatch hp, List<int> array)
        {
        ...
        private static void GetHeightData(CompactHeightfield chf, IndexedPolygon poly, int npoly, Int3[] verts, int bs, HeightPatch hp, List<int> queue, int region)
        {
            // Note: Reads to the compact heightfield are offset by border size (bs)
            // since border size offset is already removed from the polymesh vertices.
