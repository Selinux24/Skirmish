...
namespace Engine.PathFinding.RecastNavigation.Recast
{
    ...
    class ContourSet
    {
        public static ContourSet BuildContours(CompactHeightfield chf, float maxError, int maxEdgeLen, BuildContoursFlagTypes buildFlags)
        {
            int w = chf.Width;
            int h = chf.Height;
            int bSize = chf.BorderSize;
            int maxContours = Math.Max(chf.MaxRegions, 8);

            var boundsmin = chf.BoundingBox.Minimum;
            var boundsmax = chf.BoundingBox.Maximum;
            if (bSize > 0)
            {
                // If the heightfield was build with bordersize, remove the offset.
                float pad = bSize * chf.CS;
                boundsmin.X += pad;
                boundsmin.Z += pad;
                boundsmax.X -= pad;
                boundsmax.Z -= pad;
            ...
            for (int y = 0; y < h; ++y)
            {
                for (int x = 0; x < w; ++x)
                {
                    ...
                    for (int i = c.Index, ni = (c.Index + c.Count); i < ni; ++i)
                    {
                        int res = 0;
                        var s = chf.Spans[i];
                        if (chf.Spans[i].Reg == 0 || (chf.Spans[i].Reg & RecastUtils.RC_BORDER_REG) != 0)
                        {
                            flags[i] = 0;
                            continue;
                        }
            ...
            for (int y = 0; y < h; ++y)
            {
                for (int x = 0; x < w; ++x)
                {
                    ...
                    for (int i = c.Index, ni = (c.Index + c.Count); i < ni; ++i)
                    {
                        ...
                        if (reg == 0 || (reg & RecastUtils.RC_BORDER_REG) != 0)
                        {
                            continue;
                        }
                        ...
                        chf.WalkContour(x, y, i, flags, out verts);
                        ...
                        if (simplified.Count >= 3)
                        {
                            ...
                            var cont = new Contour
                            {
                                ...
                                NRawVerts = verts.Count,
                                RawVerts = verts.ToArray(),
                                Region = reg,
                                Area = area
                            };
                ...
                if (nholes > 0)
                {
                    ...
                    for (int i = 0; i < cset.NConts; ++i)
                    {
                        ...
                        if (winding[i] > 0)
                        {
                            ...
                            {
                                Console.WriteLine($"Multiple outlines for region {cont.Region}");
                            }
                            regions[cont.Region].Outline = cont;
                        }
                        else
                        {
                            regions[cont.Region].NHoles++;
                        }
                    }
                    int index = 0;
        ...
        private static void SimplifyContour(List<Int4> points, List<Int4> simplified, float maxError, int maxEdgeLen, BuildContoursFlagTypes buildFlags)
        {
            ...
            {
                if ((points[i].W & RecastUtils.RC_CONTOUR_REG_MASK) != 0)
                {
                    hasConnections = true;
                    break;
                }
            ...
            if (hasConnections)
            {
                ...
                {
                    int ii = (i + 1) % ni;
                    bool differentRegs = (points[i].W & RecastUtils.RC_CONTOUR_REG_MASK) != (points[ii].W & RecastUtils.RC_CONTOUR_REG_MASK);
                    bool areaBorders = (points[i].W & RecastUtils.RC_AREA_BORDER) != (points[ii].W & RecastUtils.RC_AREA_BORDER);
            ...
            for (int i = 0; i < simplified.Count;)
            {
                ...
                if ((points[ci].W & RecastUtils.RC_CONTOUR_REG_MASK) == 0 ||
                    (points[ci].W & RecastUtils.RC_AREA_BORDER) != 0)
                {
                    while (ci != endi)
                    {
            ...
            if (maxEdgeLen > 0 && (buildFlags & (BuildContoursFlagTypes.TessellateWallEdges | BuildContoursFlagTypes.TessellateAreaEdges)) != 0)
            {
                for (int i = 0; i < simplified.Count;)
                {
                    int ii = (i + 1) % (simplified.Count);
                    ...
                    bool tess = false;
                    // Wall edges.
                    if ((buildFlags & BuildContoursFlagTypes.TessellateWallEdges) != 0 &&
                        (points[ci].W & RecastUtils.RC_CONTOUR_REG_MASK) == 0)
                    {
                        tess = true;
                    }
                    // Edges between areas.
                    if ((buildFlags & BuildContoursFlagTypes.TessellateAreaEdges) != 0 &&
                        (points[ci].W & RecastUtils.RC_AREA_BORDER) != 0)
                    {
                        tess = true;
                    }
            ...
            for (int i = 0; i < simplified.Count; ++i)
            {
                ...
                int ai = (sv.W + 1) % pn;
                int bi = sv.W;
                sv.W = (points[ai].W & (RecastUtils.RC_CONTOUR_REG_MASK | RecastUtils.RC_AREA_BORDER)) | (points[bi].W & RecastUtils.RC_BORDER_VERTEX);
                simplified[i] = sv;
            }
        }
