...
namespace Engine.PathFinding.RecastNavigation.Recast
{
    ...
    class CompactHeightfield
    {
        ...
        private static void DelaunayHull(IEnumerable<Vector3> pts, IEnumerable<int> hull, List<Int3> outTris, List<Int4> outEdges)
        {
            int npts = pts.Count();
            ...
            {
                AddEdge(ref edges, ref nedges, maxEdges, hull.ElementAt(j), hull.ElementAt(i), (int)EdgeValues.EV_HULL, (int)EdgeValues.EV_UNDEF);
            }
        ...
        private static void CompleteFacet(IEnumerable<Vector3> pts, int npts, ref Int4[] edges, ref int nedges, int maxEdges, ref int nfaces, int e)
        {
            ...
            {
                if (u == s || u == t) continue;
                if (VCross2(pts.ElementAt(s), pts.ElementAt(t), pts.ElementAt(u)) > EPS)
                {
                    if (r < 0)
                    {
        ...
        private static int AddEdge(ref Int4[] edges, ref int nedges, int maxEdges, int s, int t, int l, int r)
        {
            if (nedges >= maxEdges)
            {
        ...
        private static bool OverlapEdges(IEnumerable<Vector3> pts, IEnumerable<Int4> edges, int nedges, int s1, int t1)
        {
            for (int i = 0; i < nedges; ++i)
            {
                ...
                if (OverlapSegSeg2d(pts.ElementAt(s0), pts.ElementAt(t0), pts.ElementAt(s1), pts.ElementAt(t1)) != 0)
                {
                    return true;
                }
            }
        ...
        private static int FindEdge(IEnumerable<Int4> edges, int nedges, int s, int t)
        {
            for (int i = 0; i < nedges; i++)
            {
        ...
        private static bool CircumCircle(Vector3 p1, Vector3 p2, Vector3 p3, out Vector3 c, out float r)
        {
            float EPS = 1e-6f;
        ...
        public void BuildPolyDetail(IEnumerable<Vector3> polygon, BuildPolyDetailParams param, HeightPatch hp, out Vector3[] outVerts, out Int3[] outTris)
        {
            ...
            if (sampleDist > 0)
            {
                ...
                for (int iter = 0; iter < nsamples; ++iter)
                {
                    ...
                    edges.Clear();
                    tris.Clear();
                    DelaunayHull(verts, hull.Take(nhull), tris, edges);
                }
            }
