...
namespace AISamples.SceneCWRVirtualWorld.Dialogs
{
    class UIOpenFileDialog(Scene scene, float width, float height, int pageCount, int layer)
    {
        ...
        public async Task Initialize(string resourcesFolder, string editorFont)
        {
            ...
            selectedText = await scene.AddComponentUI<UITextArea, UITextAreaDescription>(nameof(selectedText), nameof(selectedText), fileTextDesc);
            folderText = await scene.AddComponentUI<UITextArea, UITextAreaDescription>(nameof(folderText), nameof(folderText), fileTextDesc);
            ...
            dialog = await scene.AddComponentUI<UIDialog, UIDialogDescription>(nameof(dialog), nameof(dialog), fileDialogDesc, Layer);
            ...
            pageUpButton = await scene.AddComponentUI<UIButton, UIButtonDescription>(nameof(pageUpButton), nameof(pageUpButton), filePageButtonDesc, Layer + 1);
            ...
            pageDownButton = await scene.AddComponentUI<UIButton, UIButtonDescription>(nameof(pageDownButton), nameof(pageDownButton), filePageButtonDesc, Layer + 1);
        ...
        {
            var button = await scene.AddComponentUI<UIButton, UIButtonDescription>(name, name, desc, Layer + 1);
